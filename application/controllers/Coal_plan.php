...
class Coal_plan extends Admin_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->model('coal_plan_model');
	...
	public function index() {
		...
		if ($this->data['is_can_read']) {
			...
			$this->data['content'] = 'admin/coal_plan/list_v';
		} else {
	...
	public function create() {
		if ($this->data['is_can_create']) {
			...
			$this->data['content'] = 'admin/coal_plan/create_v';
	...
	public function dataList() {
		...
		$where = [
			"YEAR(coal_plan.tanggal)" => $tahun,
			"MONTH(coal_plan.tanggal)" => $bulan,
		];

		if (!empty($location_id)) {
			$where["coal_plan.location_id"] = $location_id;
		}

		$plan = $this->coal_plan_model->getAllGroupByUnit($where);
		$tmp_datas = [];
		$data_total = ["total" => 0];
		if (!empty($plan)) {
	...
	public function input_data() {
		if ($this->data['is_can_read']) {
			...
			if ($this->form_validation->run() === TRUE) {
				...
				$where = [
					"YEAR(coal_plan.tanggal) = " => $tahun,
					"MONTH(coal_plan.tanggal) = " => sprintf('%02d', $bulan),
					"coal_plan.unit_id" => $unit,
				];
				$delete = $this->coal_plan_model->delete($where);
				...
				if (!empty($data_batch)) {
					$this->coal_plan_model->insert_batch($data_batch);
				}
	...
	public function get_data() {
		...
		$data = $this->coal_plan_model->getAllById($where);
	...
	public function cek_data() {
		...
		$data = $this->coal_plan_model->getAllById(array("coal_plan.tanggal" => $tanggal));
	...
	public function exportExcel() {
		$where = [
			"YEAR(coal_plan.tanggal)" => $this->input->get('tahun'),
			"MONTH(coal_plan.tanggal)" => $this->input->get('bulan'),
		];

		$plan = $this->coal_plan_model->getAllById($where);
	...
	public function import() {
		$this->data['content'] = 'admin/plan/import_v';
		$this->load->view('admin/layouts/page', $this->data);
	}
